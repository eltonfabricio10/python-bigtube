...
class SettingsController:
    ...
    def __init__(
        self, row_folder, btn_pick, row_version, btn_update, window_parent, text_widgets=None
    ):
        ...
        self.download_ctrl = DownloadSettingsController(
            self.window, self.widgets_map, self.row_folder, self.btn_pick
        )
    ...
    def _setup_ui_text(self, widgets):
        ...
        if "row_max_downloads" in widgets:
            ...
            widgets["row_max_downloads"].set_subtitle(
                Res.get(StringKey.PREFS_MAX_SIMULTANEOUS_DESC)
            )
        ...
        if "row_system_notifications" in widgets:
            widgets["row_system_notifications"].set_title(
                Res.get(StringKey.PREFS_NOTIFICATIONS_LABEL)
            )
            widgets["row_system_notifications"].set_subtitle(
                Res.get(StringKey.PREFS_NOTIFICATIONS_DESC)
            )
        ...
        if "row_enable_suggestions" in widgets:
            widgets["row_enable_suggestions"].set_title(
                Res.get(StringKey.PREFS_ENABLE_SUGGESTIONS_LABEL)
            )
            widgets["row_enable_suggestions"].set_subtitle(
                Res.get(StringKey.PREFS_ENABLE_SUGGESTIONS_DESC)
            )

        if "row_max_suggestions" in widgets:
            widgets["row_max_suggestions"].set_title(Res.get(StringKey.PREFS_MAX_SUGGESTIONS_LABEL))
            widgets["row_max_suggestions"].set_subtitle(
                Res.get(StringKey.PREFS_MAX_SUGGESTIONS_DESC)
            )

        if "row_clear_search_history" in widgets:
            widgets["row_clear_search_history"].set_title(
                Res.get(StringKey.BTN_CLEAR_SEARCH_HISTORY)
            )
            widgets["row_clear_search_history"].set_subtitle(
                Res.get(StringKey.BTN_CLEAR_SEARCH_HISTORY_DESC)
            )
        ...
        if "row_conv_use_source" in widgets:
            widgets["row_conv_use_source"].set_title(
                Res.get(StringKey.PREFS_CONV_SAME_FOLDER_LABEL)
            )
            widgets["row_conv_use_source"].set_subtitle(
                Res.get(StringKey.PREFS_CONV_SAME_FOLDER_DESC)
            )
        ...
        if "row_post_process" in widgets:
            ...
            self._set_entry_row_description(
                widgets["row_post_process"], Res.get(StringKey.PREFS_POST_PROCESS_DESC)
            )
            self._add_entry_row_icon(widgets["row_post_process"], "utilities-terminal-symbolic")
        if "row_cookies_file" in widgets:
            widgets["row_cookies_file"].set_title(Res.get(StringKey.PREFS_COOKIES_FILE_LABEL))
            widgets["row_cookies_file"].set_subtitle(Res.get(StringKey.PREFS_COOKIES_FILE_DESC))
        if "row_cookies_browser" in widgets:
            widgets["row_cookies_browser"].set_title(Res.get(StringKey.PREFS_COOKIES_BROWSER_LABEL))
            widgets["row_cookies_browser"].set_subtitle(
                Res.get(StringKey.PREFS_COOKIES_BROWSER_DESC)
            )
    ...
    def _setup_bindings(self, w):
        ...
        if "row_metadata" in w:
            w["row_metadata"].connect(
                "notify::active", lambda o, p: ConfigManager.set("add_metadata", o.get_active())
            )
        if "row_subtitles" in w:
            w["row_subtitles"].connect(
                "notify::active", lambda o, p: ConfigManager.set("embed_subtitles", o.get_active())
            )
        ...
        if "row_save_history" in w:
            w["row_save_history"].connect(
                "notify::active", lambda o, p: ConfigManager.set("save_history", o.get_active())
            )
        ...
        if "spin_search_limit" in w:
            w["spin_search_limit"].connect(
                "value-changed", lambda o: ConfigManager.set("search_limit", int(o.get_value()))
            )
        ...
        if "spin_max_suggestions" in w:
            w["spin_max_suggestions"].connect(
                "value-changed", lambda o: ConfigManager.set("max_suggestions", int(o.get_value()))
            )
        ...
        if "spin_rate_limit" in w:
            w["spin_rate_limit"].connect(
                "value-changed", lambda o: ConfigManager.set("rate_limit", int(o.get_value()))
            )
        if "row_post_process" in w:
            w["row_post_process"].connect(
                "apply", lambda o: ConfigManager.set("post_process_cmd", o.get_text())
            )
        if "row_cookies_file" in w:
            w["row_cookies_file"].connect("activated", self._on_cookies_file_select)
        if "btn_select_cookies_file" in w:
            w["btn_select_cookies_file"].connect("clicked", self._on_cookies_file_select)
        if "row_cookies_browser" in w:
            w["row_cookies_browser"].connect("notify::selected", self._on_cookies_browser_changed)
    ...
    def _set_entry_row_description(self, row, text: str):
        if hasattr(row, "set_placeholder_text"):
            row.set_placeholder_text(text)
        row.set_tooltip_text(text)

    def _get_available_cookie_browsers(self):
        cached = getattr(self, "_cookie_browser_cache", None)
        ...
        browsers = [("", Res.get(StringKey.OPT_NONE))]
        for value, label, commands in browser_checks:
            if any(self._browser_command_exists(command) for command in commands):
                browsers.append((value, label))
        self._cookie_browser_cache = browsers.copy()
        return browsers

    def _browser_command_exists(self, command):
        for directory in os.get_exec_path():
            path = shutil.which(command, path=directory)
            if path and not self._is_browser_tweak_wrapper(path):
                return True
        return False

    def _is_browser_tweak_wrapper(self, path):
        real_path = os.path.realpath(path)
        if os.path.basename(real_path) == "browser-tweaks-chromium-base":
            return True
        ...
        return b"browser-tweaks" in header or b"browser-optimize" in header

    def _setup_cookie_browser_model(self, row):
        browsers = self._get_available_cookie_browsers()
        current = ConfigManager.get("cookies_browser") or ""
        if current and current not in {value for value, _label in browsers}:
            browsers.append((current, current))

        self._cookie_browser_values = [value for value, _label in browsers]
        row.set_model(Gtk.StringList.new([label for _value, label in browsers]))
        row.set_selected(
            self._cookie_browser_values.index(current)
            if current in self._cookie_browser_values
            else 0
        )

    def _on_cookies_file_select(self, *_args):
        dialog = Gtk.FileDialog()
        dialog.set_title(Res.get(StringKey.PREFS_COOKIES_FILE_LABEL))
        dialog.open(self.window, None, self._on_cookies_file_selected)

    def _on_cookies_file_selected(self, dialog, result):
        try:
            ...
            ConfigManager.set("cookies_file", path)
            if "row_cookies_file" in self.widgets_map:
                self.widgets_map["row_cookies_file"].set_subtitle(path)
        except Exception as exc:
            logger.error("Error selecting cookies file: %s", exc)
            MessageManager.show(Res.get(StringKey.MSG_FOLDER_SELECT_ERROR), is_error=True)

    def _on_cookies_browser_changed(self, row, _param):
        values = getattr(self, "_cookie_browser_values", [""])
    ...
    def _load_initial_state(self):
        ...
        if "row_conv_folder" in w:
            w["row_conv_folder"].set_subtitle(ConfigManager.get("converter_path"))
        if "spin_fragments" in w:
        ...
        if "row_cookies_file" in w:
            path = ConfigManager.get("cookies_file") or Res.get(StringKey.PREFS_COOKIES_FILE_DESC)
            w["row_cookies_file"].set_subtitle(path)
        if "row_cookies_browser" in w:
