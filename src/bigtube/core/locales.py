...
class StringKey(Enum):
    ...
    PREFS_FRAGMENTS_LABEL = N_("Concurrent Fragments")
    PREFS_FRAGMENTS_DESC = N_("Number of parallel fragments per download (1-16)")
    PREFS_RATE_LIMIT_LABEL = N_("Download Speed Limit (KB/s)")
    PREFS_RATE_LIMIT_DESC = N_("Set to 0 for unlimited")
    PREFS_NOTIFICATIONS_LABEL = N_("System Notifications")
    PREFS_EXPORT_HISTORY = N_("Export History")
    ...
    PREFS_COLOR_SCHEME_DESC = N_("Select a custom accent color for the interface")
    PREFS_VERSION_DESC = N_("Check for updates to yt-dlp and components")
    PREFS_FOLDER_DESC = N_("Directory where downloads will be saved")
    PREFS_QUALITY_DESC = N_("Default resolution or format for new downloads")
    PREFS_MAX_SIMULTANEOUS_DESC = N_("How many videos can download at the same time")
    PREFS_METADATA_DESC = N_("Add artist, album, and covers to downloaded files")
    PREFS_SUBTITLES_DESC = N_("Attempt to fetch and embed subtitles if found")
    PREFS_NOTIFICATIONS_DESC = N_("Show system-level alerts for download events")
    PREFS_CLIPBOARD_DESC = N_("Automatically detect video links in your clipboard")
    PREFS_SAVE_HISTORY_DESC = N_("Keep a log of your past downloads locally")
    PREFS_AUTO_CLEAR_DESC = N_("Wipe all history and temporary data on exit")
    PREFS_EXPORT_HISTORY_DESC = N_("Save your download history to a file")
    PREFS_IMPORT_HISTORY_DESC = N_("Restore history from a backup file")
    PREFS_CLEAR_DATA_DESC = N_("Permanently delete all settings and history")
    PREFS_SAVE_SEARCH_DESC = N_("Keep track of your search queries locally")
    PREFS_SEARCH_LIMIT_DESC = N_("Maximum number of items in search results")
    PREFS_ENABLE_SUGGESTIONS_DESC = N_("Show suggestions while typing in search bar")
    PREFS_MAX_SUGGESTIONS_DESC = N_("Number of suggestions to display at once")
    BTN_CLEAR_SEARCH_HISTORY_DESC = N_("Delete all previous search entries")
    PREFS_CONV_FOLDER_DESC = N_("Target directory for converted files")
    PREFS_CONV_HISTORY_DESC = N_("Keep a history of converted media files")
    PREFS_CONV_SAME_FOLDER_DESC = N_("Save converted files next to originals")
